...
#get variant mapping
#filter counts for just indels
indelcounts <- counts %>% filter(str_detect(name, regex("insert_|delete_", ignore_case = TRUE)))
...
#filter key for just indels
indelkey <- key %>% filter(str_detect(ID, regex("insert_|delete_", ignore_case = TRUE)))

vardf <- create_var_df(indelcounts, indelkey)
...
write_tsv(top_var, "bcalm_indels_labeled_neuronss_10bcthres.tsv")
