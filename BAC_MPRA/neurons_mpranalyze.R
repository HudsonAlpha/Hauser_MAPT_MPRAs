...
#filter rows for bins where both sums are greater than or equal to 25
...
write.table(alpha, file="alpha_analysis2_prefilt.tsv", quote=FALSE, sep='\t', col.names=NA)

png("alpha_boxplot_analysis2_prefilt.png", width=800, height=800)
...
write.table(res.cond.labeled, file=paste0("testEmpirical_alpha_", mycondition, "_results_labeled_analysis2_prefilt.tsv"), quote=FALSE, sep='\t', col.names=NA)

png("all_pval_hist_analysis2_prefilt.png", width = 800, height = 800)
